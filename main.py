...
sheet = wb.active
sheet.title = 'Чеки'
titles = ('Тип документа', 'Номер', 'Дата', 'Поставщик', 'Счет-фактура', 'Номенклатура', 'Описание', 'Количество', 'Цена', 'Сумма')
...
for item in js_check:
    ...
    supplier = item['ticket']['document']['receipt']['user']
    for itemname in item['ticket']['document']['receipt']['items']:
        ...
        sheet.cell(row=table_row, column=4).value = supplier
